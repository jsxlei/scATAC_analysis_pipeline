...
def get_mean_shap(args):
    ...
    outfile_h5 = os.path.join(outdir, '.'.join([cluster, 'mean', 'shap', 'variant_shap',
                                                shap_type, 'h5']))
    ...
    for fld in range(5):
        ...
        infile = os.path.join(shap_dir, cluster, fold, 'shap',
                                '.'.join(['.variant_shap',
                                        shap_type, 'h5']))
...
parser.add_argument('--shap_dir', type=str, help='Base directory containing fold{0,1,2,3,4}')
parser.add_argument('--shap_types', type=str, nargs='+', default=["counts"], help='Types of SHAP scores to average: "counts" or "profile"')
args = parser.parse_args()

if __name__ == "__main__":
    shap_dir = args.shap_dir
    shap_types = args.shap_types
