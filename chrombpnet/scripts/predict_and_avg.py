...
def parse_args():
    ...
    parser.add_argument("-cmb", "--chrombpnet-model-nb", type=str, action="append", required=True, help="Path to chrombpnet model h5")
...
def main(args):
    ...
    inputlens = []
    outputlens = []
    for model_path in args.chrombpnet_model_nb:
    ...
    sum_logcounts = None
    for model in models:
        pred_logits_wo_bias, pred_logcts_wo_bias = model.predict([seqs],
        ...
        if sum_logits is None:
            sum_logits = pred_logits_wo_bias.copy()
            sum_logcounts = pred_logcts_wo_bias.copy()
        else:
            sum_logits = sum_logits + pred_logits_wo_bias
            sum_logcounts = sum_logcounts + pred_logcts_wo_bias
